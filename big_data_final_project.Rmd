...
```{r}
fit.1 <- glm(readmitted_binary ~ 
                race + gender + age + admission_type_id +  discharge_disposition_id + admission_source_id + time_in_hospital +
...
change+                   diabetesMed, data = diab_dt, family = "binomial")
```
...
```{r}
a1_readmit_complete_dt <- na.omit(a1_readmit)
```
